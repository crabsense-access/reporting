...
import { fetchClicksPareto, type SeoClicksParetoResult } from "@/lib/gsc/reports";
...
// GET /api/dashboard/[clientId]/seo/clicks/pareto?from=...&to=...
// Concentración de clicks por keyword (Pareto) — ver fetchClicksPareto en
...
export async function GET(
  ...
  if (!dataSource) {
    const payload: SeoClicksParetoResponse = { connected: false, brandRegex: null, result: null };
  ...
  try {
    const result = await withCache(
      { clientId, source: "search_console", query: "fetchClicksPareto", params: { siteUrl: config.site_url, from: range.from, to: range.to } },
      () => fetchClicksPareto(config.site_url, range, previousRange)
    );
    const payload: SeoClicksParetoResponse = { connected: true, brandRegex: config.brand_regex ?? null, result };
    return NextResponse.json(payload);
  } catch (error) {
    console.error("Error consultando la concentración de clicks (Pareto)", error);
