...
import { fetchImpressionsTrend, type SeoImpressionsResult } from "@/lib/gsc/reports";
...
// GET /api/dashboard/[clientId]/seo/impressions?from=...&to=...&granularity=day|week|month
// "Impresiones" (SEO > Visión General v2) — ver fetchImpressionsTrend en
...
export async function GET(
  ...
  if (!dataSource) {
    const payload: SeoImpressionsResponse = { connected: false, result: null };
  ...
  try {
    const result = await withCache(
      { clientId, source: "search_console", query: "fetchImpressionsTrend", params: { siteUrl: config.site_url, from: range.from, to: range.to, granularity } },
      () => fetchImpressionsTrend(config.site_url, range, previousRange, granularity)
    );
    const payload: SeoImpressionsResponse = { connected: true, result };
    return NextResponse.json(payload);
  } catch (error) {
    console.error("Error consultando impresiones", error);
