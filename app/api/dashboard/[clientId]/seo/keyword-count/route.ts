...
import { fetchKeywordCount, type SeoKeywordCountResult } from "@/lib/gsc/reports";
...
// GET /api/dashboard/[clientId]/seo/keyword-count?from=...&to=...&granularity=day|week|month
// "Cantidad de Keywords" (SEO > Visión General v2) — ver fetchKeywordCount
...
export async function GET(
  ...
  if (!dataSource) {
    const payload: SeoKeywordCountResponse = { connected: false, result: null };
  ...
  try {
    const result = await withCache(
      { clientId, source: "search_console", query: "fetchKeywordCount", params: { siteUrl: config.site_url, from: range.from, to: range.to, granularity } },
      () => fetchKeywordCount(config.site_url, range, previousRange, granularity)
    );
    const payload: SeoKeywordCountResponse = { connected: true, result };
    return NextResponse.json(payload);
  } catch (error) {
    console.error("Error consultando cantidad de keywords", error);
    return NextResponse.json({ error: "No se pudo calcular la cantidad de keywords." }, { status: 502 });
  }
}
