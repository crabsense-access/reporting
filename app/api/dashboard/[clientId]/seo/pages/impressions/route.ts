...
import { fetchPagesImpressionsTrend, type SeoPagesImpressionsResult } from "@/lib/gsc/reports";
...
// GET /api/dashboard/[clientId]/seo/pages/impressions?from=...&to=...&granularity=day|week|month&segment=all|institucional|blog-portada|blog-notas
// "Impresiones" (SEO > Páginas > Concentración) — ver fetchPagesImpressionsTrend
// en lib/gsc/reports.ts. Respeta el segmento elegido en el selector
...
export async function GET(
  ...
  if (!dataSource) {
    const payload: SeoPagesImpressionsResponse = { connected: false, result: null };
    ...
    const result = await withCache(
      {
        ...
        query: "fetchPagesImpressionsTrend",
        params: { segmentLabel: segment.label, siteUrl: segment.siteUrl, dimensionFilterGroups: segment.dimensionFilterGroups, from: range.from, to: range.to, granularity },
      },
      () => fetchPagesImpressionsTrend(segment, range, previousRange, granularity)
    );
    const payload: SeoPagesImpressionsResponse = { connected: true, result };
    return NextResponse.json(payload);
  } catch (error) {
    console.error("Error consultando impresiones de páginas", error);
    return NextResponse.json({ error: "No se pudo calcular la tendencia de impresiones." }, { status: 502 });
  }
}
