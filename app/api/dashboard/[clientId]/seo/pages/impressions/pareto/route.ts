...
import { fetchPagesImpressionsPareto, type SeoPagesImpressionsParetoResult } from "@/lib/gsc/reports";
...
// GET /api/dashboard/[clientId]/seo/pages/impressions/pareto?from=...&to=...&segment=all|institucional|blog-portada|blog-notas
// Concentración de impresiones por página (Pareto) — ver
// fetchPagesImpressionsPareto en lib/gsc/reports.ts. Respeta el segmento
// elegido en el selector transversal de la hoja. Sin pill de marca (no
// aplica a páginas).
export async function GET(
  ...
  if (!dataSource) {
    const payload: SeoPagesImpressionsParetoResponse = { connected: false, result: null };
    ...
    const result = await withCache(
      {
        ...
        query: "fetchPagesImpressionsPareto",
      ...
      () => fetchPagesImpressionsPareto(segment, range, previousRange)
    );
    const payload: SeoPagesImpressionsParetoResponse = { connected: true, result };
    return NextResponse.json(payload);
  } catch (error) {
    console.error("Error consultando la concentración de impresiones por página (Pareto)", error);
