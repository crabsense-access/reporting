...
import { fetchPageCount, type SeoPageCountResult } from "@/lib/gsc/reports";
...
// GET /api/dashboard/[clientId]/seo/pages/count?from=...&to=...&granularity=day|week|month&segment=all|institucional|blog-portada|blog-notas
// "Páginas activas" (SEO > Páginas > Resumen) — ver fetchPageCount en
...
export async function GET(
  ...
  if (!dataSource) {
    const payload: SeoPageCountResponse = { connected: false, result: null };
  ...
  try {
    const result = await withCache(
      ...
      () => fetchPageCount(segment, range, previousRange, granularity)
    );
    const payload: SeoPageCountResponse = { connected: true, result };
    return NextResponse.json(payload);
  } catch (error) {
    console.error("Error consultando páginas activas", error);
    return NextResponse.json({ error: "No se pudo calcular la cantidad de páginas." }, { status: 502 });
  }
}
