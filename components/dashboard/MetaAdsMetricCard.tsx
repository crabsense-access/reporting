...
import { formatBucketLabel } from "@/components/dashboard/MetaAdsTrendChart";
...
import type { MetaAdsTimeSeriesPoint } from "@/lib/meta-ads/reports";
...
// Mismo criterio "higherIsBetter" que ya usa buildMetaAdsInsights
// (lib/insights/dashboard-insights.ts) para las mismas 8 métricas
// seleccionables — repetido acá porque esto es sobre color/sentimiento de
// UI, no sobre el texto del insight, pero deben leer igual en toda la
// página. Devuelve "neutral" si no hay período anterior con qué comparar.
...
export function buildMiniSeries(points: MetaAdsTimeSeriesPoint[], key: MetaAdsMetricKey) {
...
interface MetaAdsMetricCardProps {
  ...
  color: string | null;
  onSelect: (key: MetaAdsMetricKey) => void;
}

// Un scorecard autocontenido: valor + variación vs. período anterior (%
// y nominal) + mini gráfico de barras (sin valores a la vista, solo al
// hacer hover) + un insight real generado por LLM (cruza las 8 dimensiones
// de desglose, no una simple descripción de variación — ver
// lib/insights/meta-ads-llm-insight.ts) — todo adentro de la misma Card,
// nada se muestra por fuera. El insight arranca oculto por defecto: un
// botón lo despliega/oculta (en vez de mostrarse solo y descartarse para
// siempre). El click en cualquier parte de la card (menos ese botón) la
// selecciona/deselecciona para el gráfico grande de comparación de abajo.
