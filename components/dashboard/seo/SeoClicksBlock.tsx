...
import type { SeoClicksBucket, SeoClicksParetoKeywordStat, SeoClicksParetoResult, SeoClicksResult } from "@/lib/gsc/reports";
...
interface SeoClicksParetoResponse {
  connected: boolean;
  brandRegex: string | null;
  result: SeoClicksParetoResult | null;
}

interface SeoClicksBlockProps {
  clientId: string;
...
async function fetchClicks(clientId: string, range: DateRangeValue, granularity: Granularity): Promise<SeoClicksResult | null> {
  const params = new URLSearchParams({ from: range.from, to: range.to, granularity });
  const response = await fetch(`/api/dashboard/${clientId}/seo/clicks?${params.toString()}`);
  if (!response.ok) {
    const body = await response.json().catch(() => null);
    throw new Error(body?.error ?? "No se pudo cargar los clicks.");
  }
  const json = (await response.json()) as SeoClicksResponse;
  return json.result;
}

// El sentiment del insight de Pareto se decide por el NIVEL absoluto de
// concentración (X), no por cómo varió vs. el período anterior — pasar de
// 0% a 1% es una mejora relativa pero sigue siendo una dependencia crítica
// de tráfico, así que generateInsight() (pensado para variación) clasificaba
// mal este caso como "positivo". N=1 es un caso aparte, más grave que el
// resto del rango "negative".
...
function paretoConcentrationText(
  result: SeoClicksParetoResult,
  topKeyword: SeoClicksParetoKeywordStat | null,
  ...
  if (N === 1 && topKeyword) {
    return `Todo tu tráfico relevante depende de una sola keyword: **'${topKeyword.key}'**, responsable del **${topKeywordPct}%** de tus clicks. Es una dependencia crítica — conviene diversificar activamente.`;
  }
  if (X < 10) {
    return `Tu tráfico está muy concentrado: solo **${N}** de tus **${T}** keywords (**${X}%**) generan el 80% de tus clicks. Si alguna de estas keywords pierde posiciones, gran parte de tu tráfico queda en riesgo.`;
  }
  if (X < 25) {
    return `Tu tráfico está moderadamente concentrado: **${N}** de tus **${T}** keywords (**${X}%**) generan el 80% de tus clicks.${
      topKeyword ? ` Tu keyword más fuerte, **'${topKeyword.key}'**, aporta el **${topKeywordPct}%** de ese total.` : ""
    }`;
  }
  return `Tu tráfico está bien diversificado: hacen falta **${N}** de tus **${T}** keywords (**${X}%**) para llegar al 80% de tus clicks.${
    topKeyword ? ` Tu keyword más fuerte, **'${topKeyword.key}'**, aporta el **${topKeywordPct}%** de ese total.` : ""
  }`;
}
...
// clicks desc desde el backend) para el desglose de Pareto — solo
// Keyword + Clicks, separada en dos grupos con scroll independiente: las
// keywords del corte del 80% arriba, el resto (cola larga) abajo, con un
// borde superior remarcado + label inline en la primera fila del segundo
        ...
        <>
          ...
          <div className="flex items-center gap-2 py-1">
            <span className="inline-block shrink-0 rounded-full bg-emerald-600 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
              80% de clicks ({formatNumber(topKeywords.length)} Kws) ↑
            </span>
...
// Bloque "Clicks" (SEO > Visión General v2, debajo de Keywords Nuevas/
// Perdidas) — scorecard con MetricScorecardGroup (Prompt 92, refactor de
// Prompt 73) más el análisis de Pareto de concentración de clicks por
// keyword sin cambios, con insight de reglas (generateInsight, no LLM)
// comparando la concentración de este período contra la del período
// anterior. El scorecard y el bloque de Pareto pasan a ser 2 Cards
// separadas (antes compartían una sola con un divisor interno) porque
// MetricScorecardGroup ya trae su propia Card por tarjeta.
export function SeoClicksBlock({ clientId, range, defaultGranularity }: SeoClicksBlockProps) {
  const [result, setResult] = useState<SeoClicksResult | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [paretoResult, setParetoResult] = useState<SeoClicksParetoResult | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetchClicks(clientId, range, defaultGranularity)
      .then((res) => {
  ...
  const fetchTrend = useCallback(
    (granularity: Granularity): Promise<ScorecardTrendPoint[]> =>
      fetchClicks(clientId, range, granularity).then((res) =>
  ...
  useEffect(() => {
    ...
    fetch(`/api/dashboard/${clientId}/seo/clicks/pareto?${params.toString()}`)
      .then(async (response) => {
        if (!response.ok) {
          const body = await response.json().catch(() => null);
          throw new Error(body?.error ?? "No se pudo calcular la concentración de clicks.");
        }
        return (await response.json()) as SeoClicksParetoResponse;
      })
      .then((json) => {
        if (!cancelled) {
  ...
  // sentiment/color se decide por el NIVEL absoluto de X (paretoConcentration
  // Sentiment), no por la variación vs. el período anterior — generateInsight()
  // solo se usa acá para redactar la cláusula de cierre sobre esa variación
  // (banco de frases ya existente), IGNORANDO el sentiment que devuelve.
  const topKeyword = paretoResult?.topKeywords[0] ?? null;
  const topKeywordPct =
    topKeyword && paretoResult && paretoResult.totalClicks > 0 ? Math.round((topKeyword.clicks / paretoResult.totalClicks) * 100) : 0;

  const paretoComparisonInsight = paretoResult
    ? generateInsight({
        label: "Concentración de clicks (Pareto)",
          ...
          {paretoError ? (
            ...
            <>
              ...
              <div className="h-32 overflow-y-auto">
                {paretoInsight && <InsightsList insights={[paretoInsight]} sectionKey="seo" keyPrefix="clicks_pareto" />}
              </div>

              <ParetoTable
                topKeywords={paretoResult.topKeywords}
                restKeywords={paretoResult.restKeywords}
                totalClicks={paretoResult.totalClicks}
