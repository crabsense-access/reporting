...
import type { SeoImpressionsBucket, SeoPageParetoStat, SeoPagesImpressionsParetoResult, SeoPagesImpressionsResult } from "@/lib/gsc/reports";
import type { SeoPageSegmentKey } from "@/lib/gsc/segments";

interface SeoPagesImpressionsResponse {
  connected: boolean;
  result: SeoPagesImpressionsResult | null;
}

interface SeoPagesImpressionsParetoResponse {
  connected: boolean;
  result: SeoPagesImpressionsParetoResult | null;
}

interface SeoPagesImpressionsParetoBlockProps {
  clientId: string;
...
async function fetchPagesImpressions(
  clientId: string,
  range: DateRangeValue,
  segment: SeoPageSegmentKey,
  granularity: Granularity
): Promise<SeoPagesImpressionsResult | null> {
  const params = new URLSearchParams({ from: range.from, to: range.to, granularity, segment });
  const response = await fetch(`/api/dashboard/${clientId}/seo/pages/impressions?${params.toString()}`);
  if (!response.ok) {
    const body = await response.json().catch(() => null);
    throw new Error(body?.error ?? "No se pudieron cargar las impresiones.");
  }
  const json = (await response.json()) as SeoPagesImpressionsResponse;
  return json.result;
}
...
// criterio que el bloque de Clicks de esta sección (ver
// SeoPagesClicksParetoBlock.tsx), duplicado acá a propósito para que este
// bloque no dependa de aquel.
...
function paretoConcentrationText(result: SeoPagesImpressionsParetoResult, topPage: SeoPageParetoStat | null, topPagePct: number): string {
  const { cutoffCount: N, totalPages: T, cutoffPagesPercent: X } = result;

  if (T === 0) return "Sin actividad de páginas en este período.";

  if (N === 1 && topPage) {
    return `Todo tu tráfico relevante depende de una sola página: **'${topPage.key}'**, responsable del **${topPagePct}%** de tus impresiones. Es una dependencia crítica — conviene diversificar activamente.`;
  }
  if (X < 10) {
    return `Tu tráfico está muy concentrado: solo **${N}** de tus **${T}** páginas (**${X}%**) generan el 80% de tus impresiones. Si alguna de estas páginas pierde posiciones, gran parte de tu tráfico queda en riesgo.`;
  }
  if (X < 25) {
    return `Tu tráfico está moderadamente concentrado: **${N}** de tus **${T}** páginas (**${X}%**) generan el 80% de tus impresiones.${
      topPage ? ` Tu página más fuerte, **'${topPage.key}'**, aporta el **${topPagePct}%** de ese total.` : ""
    }`;
  }
  return `Tu tráfico está bien diversificado: hacen falta **${N}** de tus **${T}** páginas (**${X}%**) para llegar al 80% de tus impresiones.${
    topPage ? ` Tu página más fuerte, **'${topPage.key}'**, aporta el **${topPagePct}%** de ese total.` : ""
  }`;
}
...
// impresiones desc desde el backend) para el desglose de Pareto — Página +
// Impresiones, sin pill de marca (no aplica a páginas), separada en dos
// grupos con scroll independiente: las páginas del corte del 80% arriba, el
// resto (cola larga) abajo, con un borde superior remarcado + label inline
// en la primera fila del segundo grupo para marcar el corte.
        ...
        <>
          ...
          <div className="flex items-center gap-2 py-1">
            <span className="inline-block shrink-0 rounded-full bg-emerald-600 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
              80% de impresiones ({formatNumber(topPages.length)} páginas) ↑
            </span>
...
// Bloque "Impresiones" (SEO > Páginas > Concentración, a la izquierda de
// Clicks) — scorecard con MetricScorecardGroup (Prompt 92, refactor de
// Prompt 73) más el análisis de Pareto sin cambios (2 Cards separadas en
// vez de 1 con divisor interno, ver SeoClicksBlock.tsx), respetando el
// segmento elegido en el selector transversal de la hoja Páginas. Cálculo
// backend independiente, sin compartir estado con el bloque de Clicks.
export function SeoPagesImpressionsParetoBlock({ clientId, range, segment, defaultGranularity }: SeoPagesImpressionsParetoBlockProps) {
  const [result, setResult] = useState<SeoPagesImpressionsResult | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [paretoResult, setParetoResult] = useState<SeoPagesImpressionsParetoResult | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetchPagesImpressions(clientId, range, segment, defaultGranularity)
      .then((res) => {
  ...
  const fetchTrend = useCallback(
    (granularity: Granularity): Promise<ScorecardTrendPoint[]> =>
      fetchPagesImpressions(clientId, range, segment, granularity).then((res) =>
  ...
  useEffect(() => {
    ...
    fetch(`/api/dashboard/${clientId}/seo/pages/impressions/pareto?${params.toString()}`)
      .then(async (response) => {
        if (!response.ok) {
          const body = await response.json().catch(() => null);
          throw new Error(body?.error ?? "No se pudo calcular la concentración de impresiones.");
        }
        return (await response.json()) as SeoPagesImpressionsParetoResponse;
      })
      .then((json) => {
  ...
  const topPagePct =
    topPage && paretoResult && paretoResult.totalImpressions > 0 ? Math.round((topPage.impressions / paretoResult.totalImpressions) * 100) : 0;

  const paretoComparisonInsight = paretoResult
    ? generateInsight({
        label: "Concentración de impresiones por página (Pareto)",
          ...
          {paretoError ? (
            ...
            <>
              ...
              <div className="h-32 overflow-y-auto">
                {paretoInsight && <InsightsList insights={[paretoInsight]} sectionKey="seo" keyPrefix="pages_impressions_pareto" />}
              </div>

              <ParetoTable topPages={paretoResult.topPages} restPages={paretoResult.restPages} totalImpressions={paretoResult.totalImpressions} />
            </>
          )}
        </CardContent>
      </Card>
