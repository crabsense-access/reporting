...
import type { GA4TimeSeriesPoint } from "@/lib/ga4/reports";

// Misma paleta categórica validada que Meta Ads / Google Ads.
export const METRIC_SLOT_COLORS = ["#2a78d6", "#eb6834"] as const;

export interface GA4ChartMetric {
...
interface GA4TrendChartProps {
  current: GA4TimeSeriesPoint[];
  previous: GA4TimeSeriesPoint[];
  metrics: GA4ChartMetric[];
  loading?: boolean;
}

export function formatBucketLabel(date: string): string {
  if (!date) return "";
  // Los buckets semanales/mensuales de GA4 (yyyy-Www / yyyy-MM) no son
  // fechas ISO parseables — se muestran tal cual, solo el bucket diario
  // (yyyy-MM-dd) se formatea "d MMM".
...
function buildSingleMetricData(current: GA4TimeSeriesPoint[], previous: GA4TimeSeriesPoint[], metricKey: GA4BasicMetricKey) {
...
// refleje una relación real entre las métricas. Solo se grafica el período
// actual acá — el período anterior (línea tenue) solo tiene sentido con 1
// sola métrica seleccionada, con 2 ejes distintos ya se vuelve demasiado
// para leer (4 líneas).
function buildDualAxisData(current: GA4TimeSeriesPoint[], metrics: GA4ChartMetric[]) {
...
export function GA4TrendChart({ current, previous, metrics, loading }: GA4TrendChartProps) {
  if (loading) {
