...
import type { GoogleAdsPeriodMetrics, GoogleAdsTimeSeriesPoint } from "@/lib/google-ads/reports";

export type GoogleAdsMetricKey = keyof GoogleAdsPeriodMetrics;

// Misma paleta categórica validada que usa Meta Ads (METRIC_SLOT_COLORS en
// MetaAdsTrendChart) — se repite acá en vez de importarla para que Google
// Ads y Meta Ads no compartan un módulo sin relación entre sí, pero el
// criterio de color por posición de selección (1ª = azul, 2ª = naranja) es
// el mismo en ambos tableros.
export const METRIC_SLOT_COLORS = ["#2a78d6", "#eb6834"] as const;

export interface GoogleAdsChartMetric {
...
interface GoogleAdsTrendChartProps {
  current: GoogleAdsTimeSeriesPoint[];
  previous: GoogleAdsTimeSeriesPoint[];
  metrics: GoogleAdsChartMetric[];
  loading?: boolean;
}
...
function buildDualAxisData(current: GoogleAdsTimeSeriesPoint[], metrics: GoogleAdsChartMetric[]) {
...
export function GoogleAdsTrendChart({ current, previous, metrics, loading }: GoogleAdsTrendChartProps) {
  if (loading) {
